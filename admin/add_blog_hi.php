...
if(isset($_REQUEST['add']))
{
   ...
   if($check_count==0)
   {
	   $uniql       = uniqid($uniql);
		...
		$or_val          = $F($Q($link,"SELECT * FROM `higli` ORDER BY `order` DESC LIMIT 0,1"));
	    $up_val          = $or_val['order']+1;
	
        $ins_img = $Q($link,"INSERT INTO `higli` (`title`,`content`,`mtitle`,`mkeyword`,`mdescription`,`bdate`,`btime`,`order`, `status`,`filefl`)
        VALUES('$title', '$content', '$mtitle','$mkeyword','$mdescription','$p_date2','$p_time','$up_val', '1','$file_names')");
									  								  
       echo "<script>window.location='view_blog_hi.php?added=1&page=$page&page_count=$page_count';</script>";
   }
   else{$erre=1;}
   }
   else{$erre2=1;}
...
<div class="container">
     ...
     <form class="form-signin2"  id="blog" novalidate name="form3" method="post" enctype="multipart/form-data">
        ...
        <h4 class="form-signin-heading">Add news</h4>
 ...
 Image:<br>
        ...
        <a href="view_blog_hi.php" class="btn btn-inverse">cancel</a>
      </form>
