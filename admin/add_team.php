...
$filefl       = str_replace(" ","-",$_FILES['filefl']['name']);	
	...
	$checktitle     = $Q($link,"SELECT * FROM `teams` WHERE `tname` = '$title' ");
   ...
   if($check_count==0)
   {
		...
		$or_val          = $F($Q($link,"SELECT * FROM `teams` ORDER BY `orderi` DESC LIMIT 0,1"));
	    $up_val          = $or_val['orderi']+1;
	
        $ins_img = $Q($link,"INSERT INTO `teams` (`tname`,`tcontent`,`tno`,`filefl`,`orderi`, `status`)
        VALUES('$title', '$content', '$tno','$file_names','$up_val', '1')");
									  								  
       echo "<script>window.location='view_team.php?added=1&page=$page&page_count=$page_count&catds=$catds';</script>";
   }
   else{$erre=1;}
   }
   else{$erre2=1;}
...
<div class="container">
     ...
     <form class="form-signin2"  id="team" novalidate name="form3" method="post" enctype="multipart/form-data">
        ...
        <h4 class="form-signin-heading">Add Team</h4>
        
     
            <table width="1024" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="500">Enter Title:
        <input type="text" class="input-block-level" placeholder="Enter Title..." maxlength="100" name="tname" ></td>
    <td width="524" align="right"><input type="submit" class="btn btn-inverse"  name="add" value="SAVE">
        <a href="view_team.php" class="btn btn-inverse">cancel</a></td>
  </tr>
</table>

        <p>
        Enter team no:
        <input type="text" class="input-block-level" placeholder="Team No..." maxlength="5" name="tno" >
        </p>
        ...
        <a href="view_team.php" class="btn btn-inverse">cancel</a>
      </form>
