...
{   
	...
	$str_img =   $F($Q($link,"SELECT * FROM `blogs` WHERE `id` = '$edit' "));
	
	if($str_img['title']==$title)
	{
	$asi_ty  = $Q($link,"UPDATE `blogs` SET 
     `title` = '$title',
	`content` = '$content', 
	`mtitle` = '$mtitle',
	`mkeyword` = '$mkeyword',
    ...
    echo "<script>window.location='view_blog.php?update=1&page=$page&page_count=$page_count&catds=$catds';</script>";
	}
	...
	`mdescription` = '$mdescription'
	  ...
	  else{echo "<script>window.location='edit_blog.php?erre=1&page=$page&page_count=$page_count&edit=$edit&catds=$catds';</script>";}
	}
    ...
    if($filefl!='')
	{
        ...
        $img_dte  = $Q($link,"UPDATE `blogs` SET `filefl` = '$file_names' WHERE `id` = '$edit' ");
									  
									  
    }
}

if($edit!='') {$der = $F($Q($link,"SELECT * FROM `blogs` WHERE `id` = '$edit'")); }
...
<div class="container">
     ...
     <form class="form-signin2"  id="blogedit" novalidate name="form3" method="post" enctype="multipart/form-data">
        ...
        <h4 class="form-signin-heading">Edit highlights for <?php echo $eventdatarows['title'];?></h4>
  ...
  <p> 
        ...
        <a href="view_blog.php?page=<?php echo $page; ?>&page_count=<?php echo $page_count; ?>&sea_key=<?php echo $sea_key; ?>&catds=<?php echo $catds;?>" class="btn btn-inverse">cancel</a>
      </form>
