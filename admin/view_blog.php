...
if($approve!='')
{
	$approved_str="UPDATE `blogs` SET `status` = '0' WHERE `id` = '$approve'";
	$Q($link,$approved_str);
	echo "<script>window.location='view_blog.php?approved=1&page=$page&page_count=$page_count&catds=$catds&subcatds=$subcatds';</script>";
}

if($reject!='')
{
	$rejected_str="UPDATE `blogs` SET `status` = '1' WHERE `id` = '$reject'";
	$Q($link,$rejected_str);
	echo "<script>window.location='view_blog.php?rejected=1&page=$page&page_count=$page_count&catds=$catds&subcatds=$subcatds';</script>";
}


if($del!='')
{	
			$str2       = $Q($link,"SELECT * FROM `blogs` WHERE `id`= '$del'");

			//$image2  ="../pimages/".$row['filefl'];
			//@unlink($image2);
			$str_del2="DELETE  FROM `blogs` WHERE `id`= '$del'";
			$Q($link,$str_del2);

echo "<script>window.location='view_blog.php?delete=1&page=$page&page_count=$page_count';</script>";
}
...
<div class="container">
      ...
      <?php require_once 'common/menu.php';?>
		 	...
		 	<h4 class="form-signin-heading">View Highlights for <?php echo $eventdatarows['title'];?> </h4>
            <div align="right">
            <a href="add_blog.php?catds=<?php echo $catds;?>" class="btn btn-inverse">ADD</a>
				  ...
				  if($count!=0) {
					...
					<?php
					   ...
					   $serial=0;
						...
						{
						  $serial++;
						 //$mm = $F($Q("SELECT * FROM `blogs` WHERE `id` = '$row_list[mid]'"));
					?>
   	      ...
   	      <div class="midcon">
              <div class="container">
                  <div class="row">
                     ...
                     <div class="span3">
                     <a href="edit_blog.php?edit=<?php echo $row_list['id'];?>&page=<?php echo $page; ?>&page_count=<?php echo $page_count; ?>&sea_key=<?php echo $sea_key; ?>&catds=<?php echo $catds;?>"><i class=" icon-pencil"></i> Edit</a> /  
                     <!-- <a href="view_blog.php?del=<?php echo $row_list['id']; ?>&amp;page=<?php echo $page; ?>&amp;page_count=<?php echo $page_count; ?>&amp;sea_key=<?php echo $sea_key; ?>" onClick="return confirm('Are you sure you want to delete?')"><i class="icon-trash"></i> Delete</a>  /  -->
                    
                     <?php if($row_list['status']=='1') {?>
                      <a href="view_blog.php?approve=<?php echo $row_list['id']; ?>&amp;page=<?php echo $page; ?>&amp;page_count=<?php echo $page_count; ?>&amp;sea_key=<?php echo $sea_key; ?>"><i class="icon-ok-sign"></i></a>
                      <?php } else {?>
                      <a href="view_blog.php?reject=<?php echo $row_list['id']; ?>&amp;page=<?php echo $page; ?>&amp;page_count=<?php echo $page_count; ?>&amp;sea_key=<?php echo $sea_key; ?>" style="text-decoration:none;font-size:12px;">
         ...
         <div class="text_white">
			<?php
                      if(@$total_records > @$records_per_page)
                      {
                          ...
                          $pageL->displayLink("view_blog.php?page_count=$page_count&catds=$catds&subcatds=$subcatds", true, true, 10);
