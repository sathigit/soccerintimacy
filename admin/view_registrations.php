...
if($approve!='')
{
	$approved_str="UPDATE `register` SET `status` = '0' WHERE `id` = '$approve'";
	$Q($link,$approved_str);
	echo "<script>window.location='view_registrations.php?approved=1&page=$page&page_count=$page_count&catds=$catds&subcatds=$subcatds';</script>";
}

if($reject!='')
{
	$rejected_str="UPDATE `register` SET `status` = '1' WHERE `id` = '$reject'";
	$Q($link,$rejected_str);
	echo "<script>window.location='view_registrations.php?rejected=1&page=$page&page_count=$page_count&catds=$catds&subcatds=$subcatds';</script>";
}


if($del!='')
{	
			$str2       = $Q($link,"SELECT * FROM `register` WHERE `id`= '$del'");

			//$image2  ="../pimages/".$row['filefl'];
			//@unlink($image2);
			$str_del2="DELETE  FROM `register` WHERE `id`= '$del'";
			$Q($link,$str_del2);

echo "<script>window.location='view_registrations.php?delete=1&page=$page&page_count=$page_count';</script>";
}
...
<div class="container">
      ...
      <?php require_once 'common/menu.php';?>
		 	...
		 	<h4 class="form-signin-heading">View Registrations</h4>
		...
		<div class="midcon">
              <div class="container">
                  <div class="row">
                     ...
                     <div class="span2"><strong>Name:</strong> <i class="icon-th-list"></i></div>
                     <div class="span2"><strong>Registered date/Time</strong></div>
				  ...
				  if($count!=0) {
					...
					<?php
					   ...
					   $serial=0;
						...
						{
						  $serial++;
						 //$mm = $F($Q("SELECT * FROM `register` WHERE `id` = '$row_list[mid]'"));
					?>
   	      ...
   	      <div class="midcon">
              <div class="container">
                  <div class="row">
                     ...
                     <div class="span2"> <i class="icon-th-list"></i> 
					 	<?php echo $row_list['firstname'];?></div>
                        <div class="span2">
                   <strong><?php echo $row_list['reg_date'];?></strong> / <?php echo $row_list['reg_time'];?></div>
                     <div class="span4">
                   <?php echo $row_list['email'];?>
                    <!-- <a href="view_registrations.php?del=<?php echo $row_list['id']; ?>&amp;page=<?php echo $page; ?>&amp;page_count=<?php echo $page_count; ?>&amp;sea_key=<?php echo $sea_key; ?>" onClick="return confirm('Are you sure you want to delete?')"><i class="icon-trash"></i> Delete</a>  -->
                   - 
                  <a href="userdetails.php?user=<?php echo $row_list['id'];?>&page=<?php echo $page; ?>&page_count=<?php echo $page_count; ?>&sea_key=<?php echo $sea_key; ?>">View Details</a>
                      
                         -  <?php if($row_list['status']=='1') {?>
                      <a href="view_registrations.php?approve=<?php echo $row_list['id']; ?>&amp;page=<?php echo $page; ?>&amp;page_count=<?php echo $page_count; ?>&amp;sea_key=<?php echo $sea_key; ?>"><i class="icon-ok-sign"></i></a>
                      <?php } else {?>
                      <a href="view_registrations.php?reject=<?php echo $row_list['id']; ?>&amp;page=<?php echo $page; ?>&amp;page_count=<?php echo $page_count; ?>&amp;sea_key=<?php echo $sea_key; ?>" style="text-decoration:none;font-size:12px;">
         ...
         <div class="text_white">
			<?php
                      if(@$total_records > @$records_per_page)
                      {
                          ...
                          $pageL->displayLink("view_registrations.php?page_count=$page_count&catds=$catds&subcatds=$subcatds", true, true, 10);
