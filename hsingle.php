...
<html class="no-js" lang="zxx">
	...
	<body class="home-two">
		...
		if($eid){
		$hsinglerows2       = $F($Q($link,"SELECT * FROM `blogs` WHERE `eid` = '$eid' AND `status` = '1' ORDER BY `order` DESC"));
		$hsingle      = $F($Q($link,"SELECT * FROM `ex_mmenu` WHERE `id` = '$eid' AND `status` = '1'"));
		$hid = $hsinglerows2['id'];
		}
		
		if($hid){
		$hsinglerows       = $F($Q($link,"SELECT * FROM `blogs` WHERE `id` = '$hid' AND status = '1'"));
		$hsingle      = $F($Q($link,"SELECT * FROM `ex_mmenu` WHERE `id` = '$hsinglerows[eid]' AND `status` = '1'"));}
    			...
    			<div class="container">
    				<div class="row">
    					<div class="col-md-12 text-center">
    						<h1 class="page-title"><?php echo $hsingle['title'];?>/Highlights</h1>
    						<ul>
    							<li>
                    ...
                    <div class="col-md-8 col-sm-12">
						...
						<div class="share-section">
							<div class="row">
								<div class="col-sm-6 col-xs-12 life-style">
									...
									<span class="date">
										<i class="fa fa-calendar" aria-hidden="true"></i> <?php echo $hsinglerows['bdate'];?>
									</span>
						...
						<div class="like-section">
							...
							<div class="row">
                                ...
                                <?php
                                $geteventdata	=	$Q($link,"SELECT * FROM `blogs` WHERE `id` != '$hid' AND `status` = '1' ORDER BY `order` DESC LIMIT 3");
                                while($getdataeve	=	$F($geteventdata))
                                {
									?>
                                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-6">
									<div class="popular-post-img">
										<a href="hsingle.php?hid=<?php echo $getdataeve['id'];?>">
									...
									<h3>
										<a href="hsingle.php?id=<?php echo $getdataeve['id'];?>"><?php echo $getdataeve['title'];?></a>
									</h3>
						      ...
						      <?php } else{?>
                              <p style="color:#f00;">No Highlights found for this event</p>
                              <?php }?>                          
					</div>
                    <div class="col-md-3 col-sm-12">
						...
						<div class="sidebar-area">
                            ...
                            <div class="cate-box">
                                ...
                                <ul>
                                    <?php
                                $geteventdata2	=	$Q($link,"SELECT * FROM `ex_mmenu` WHERE `status` = '1' ORDER BY `order` DESC LIMIT 10");
                                $p=0;
								while($getdataeve2	=	$F($geteventdata2))
								//$getname    = $F($Q($link,"SELECT * FROM `blogs` WHERE `eid` = '$getdataeve2[id]' AND `status` = '1' ORDER BY `order` DESC"));
                                {
									?>
                                    <li>
                                        <i class="fa fa-angle-right" aria-hidden="true"></i> 
                                        <a href="hsingle.php?eid=<?php echo $getdataeve2['id'];?>"><?php echo $getdataeve2['title'];?></a>
                                    </li>
                                    <?php $p++;} ?>
                                </ul>
                            </div>
